...
class ProfileFrame(ctk.CTkFrame):
    ...
    def __init__(self, master, data_manager, width = 200, height = 200, corner_radius = None, border_width = None, bg_color = "transparent", fg_color = None, border_color = None, background_corner_colors = None, overwrite_preferred_drawing_method = None, **kwargs):
        ...
        self.picture = ctk.CTkImage(self.data_manager.profile_picture, size = (100, 100))
        ...
        self.username = tk.StringVar(value=self.data_manager.username)
        self.username_entry = ctk.CTkEntry(self.left_frame, textvariable=self.username)
        self.username_entry.pack(pady = 10)
        self.username_entry.bind("<Return>", lambda event: self.data_manager.save_user_data(username=self.username.get()))

        self.stats = self.data_manager.load_stats()
        self.stats_lbl = ctk.CTkLabel(self.left_frame, text = f"Odehrané hry: {self.stats['games_played']}\nVýhry: {self.stats['wins']}\nRemízy: {self.stats['draws']}\nProhry: {self.stats['losses']}")
        self.stats_lbl.pack(pady = 10)

        self.save_bt = ctk.CTkButton(self.left_frame, text = "Uložit změny", command=lambda: self.data_manager.save_user_data(username=self.username.get()))
    ...
    def reset_stats(self):
        if messagebox.askyesno("Resetovat profil", "Opravdu chcete resetovat svůj profil? Tuto akci nelze vrátit zpět!"):
            ...
            self.username.set(self.data_manager.username)
            self.picture = ctk.CTkImage(self.data_manager.profile_picture, size=(100, 100))
    ...
    def select_image(self):
        ...
        self.data_manager.save_user_data(profile_picture=img)
        self.picture = ctk.CTkImage(img, size=(100, 100))
        self.profile_picture_bt.configure(image = self.picture)

    def refresh(self):
        self.stats = self.data_manager.load_stats()
        self.stats_lbl.configure(text = f"Odehrané hry: {self.stats['games_played']}\nVýhry: {self.stats['wins']}\nRemízy: {self.stats['draws']}\nProhry: {self.stats['losses']}")